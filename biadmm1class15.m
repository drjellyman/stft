...
% Initialize Lambdas and weights
node{nearestSensor}.L = zeros(Khalf,2,node{nearestSensor}.Nlen); % These are for node m's real neighbors including itself
node{nearestSensor}.W = zeros(Khalf,node{nearestSensor}.Nlen); % Note that this includes node m itself, node m's real neighbors, and node m's virtual neighbor (m+M)
node{nearestSensor}.Lnew = zeros(Khalf,2,node{nearestSensor}.Nlen);
node{nearestSensor}.Wnew = zeros(Khalf,node{nearestSensor}.Nlen);

node{nearestSensor}.N = [nearestSensor];
node{nearestSensor}.Amn{1} = [1;0];
node{nearestSensor}.Nlen = 1;
node{m}.R = R(:,nearestSensor,nearestSensor); 
