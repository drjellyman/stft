function [ z,d] = myObservInterp( zPos,sPos,s,Fs1,Fs,nsWt )
    ...
    for a = 1:NSources
        for b = 1:NSensors
            ...
            weight = 1/(ssd); % Calculate the weight based on the source to sensor distance
%             sTemp = [zeros(ssdsInt,1); s(ssdsInt+1:end,a) ]; % Delay the source by ssdsInt
            sTemp = [zeros(ssdsInt,1); s(1:end-ssdsInt,a) ]; % Delay the source by ssdsInt
