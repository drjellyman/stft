...
maxNoNghbrs = find(sum(N,2)==-M,1)+2; % +2 so there is space for the current node and it's virtual node
N = N(1:maxNoNghbrs,:);

% Step through nodes for primal/dual update
Lambda = zeros(M,Khalf,maxNoNghbrs,2); % m x n x 2 x k ???
...
for l=2
    for m=1
        ...
        NM = myChop(N(:,m),m,m+M); % NM = list of neighbors of the current node m, note that it includes itself
        NMl = length(NM);
        ...
        XN = zeros(Khalf,NMl-1);
        for n=1:NMl-1
            XN(:,n) = X(:,l,NM(n));
        end
        % Calculate local covariance
        Rkm = zeros(Khalf,NMl-1,NMl-1);
        for k=1:Khalf
    ...
    %                 NN = myChop(N(:,NM(n)),NM(n));
                    LambdaTmp = squeeze(Lambda(NM(n),k,find(m==myChop(N(:,NM(n)),NM(n),NM(n)+M)),:));
                ...
                for n=1:NMl-1
                    ...
                    WTmp = squeeze(W(NM(n),k,1:min(find(Anm(1,:,n)==9))-1)); % 7 x 1
                    AmnTmp = squeeze(Amn(:,:,n)); % 2 x 9
                    Lambda(m,k,NM(n),:) = LambdaTmp-AnmTmp*WTmp-AmnTmp/RkmTmp*(dk+zk);
                end
        ...
        elseif m>M % Virtual nodes (Note that virtual nodes only have one neighbor)
            for k=1:Khalf
                WTmp = squeeze(Wsave(m-M,k,find(Anm(1,:,n)==9,1)-1));
                AnmTmp = [-1;1];
                AmnTmp = [1;-1];
%                 LambdaTmp = 
%                 
