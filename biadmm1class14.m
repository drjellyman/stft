...
% % Normalize the truncated sources to have equal variance of 1. 
% for ns = 2:Nsrcs
%     s{ns} = s{ns}*(sqrt((s{1}.'*s{1})/(s{ns}.'*s{ns})));
% end
% s{1}.'*s{1}
% s{2}.'*s{2}
...
%% Place sensors
M = 12^2; % M = number of sensors (note that the number of nodes will be 2*M to include virtual nodes)
...
% Random 2d placement
% xloc = (rand(M,spcDim-1)*diag(space(1:end-1))).';
% xloc = [xloc;ones(1,M)];
...
% 2d Grid placement with double up 
% xloctmp = spSize*[combvec(linspace(0,1,sqrt(M/2)),linspace(0,1,sqrt(M/2)));repmat(0,1,M/2)];
% for argh = 1:length(xloctmp)
%     xloc(:,2*argh-1) = xloctmp(:,argh);
%     xloc(:,2*argh) = xloctmp(:,argh);
% end

% 2d grid placement
xloc = spSize*[combvec(linspace(0,1,sqrt(M)), linspace(0,1,sqrt(M)));repmat(1,1,M)]+(0.01*randn(3,M));
...
for m=1:M
    node{m}.N = [find(sensd(:,m)<1.7*(spSize/(sqrt(M)-1))) ; m+M]; % m+M is the virtual node for node m. Note that the scheme for listing neighbors is smallest number to largest, which means that the virtual node is always last in the neighbor list. It also means the self connection, i.e. where node m sits in node{m}.N can be anywhere except last.
...
for k=1:Khalf
    for ii=1:M
        for jj=1:M
            ...
            R(k,ii,jj) = exp(-1i*2*pi*fdomShort(k)*(temp1-temp2)/c)/((4*pi)^2 * temp1 * temp2) ;%+ 1e-1*sinc(fdomShort(k)*(temp1-temp2)/c);
        end
    end
...
% ftmp = zeros(ITER1,M);
rho = 2; % scaling for consensus 1.5
beta = 1.5; % scaling for lambda consensus 1
alpha = 0.09; % scaling for regularization
...
%                     bk = node{m}.W(k,:)*node{m}.Amn{end}.'*node{m+M}.Amn{1}-node{m+M}.Amn{1}*node{m}.L(k,1,end);
%                     node{m+M}.Wnew(k) = -bk+sign(bk)*min(abs(bk),alpha);

%                     node{m+M}.Wnew(k) = node{m}.Amn{end}*node{m}.W(k,:).'+node{m}.L(k,1,end)-sign(node{m}.Amn{end}*node{m}.W(k,:).'+node{m}.L(k,1,end)-alpha)*alpha;

                    bk = node{m+M}.Amn{1}*(node{m}.Amn{end}*node{m}.W(k,:).'-node{m}.L(k,1,end));
%                     abs(bk)
                    node{m+M}.Wnew(k) = -bk+sign(bk)*min(abs(bk),alpha);
...
%%
% S_fvst = stft(s{1},K);
% S_fvst = S_fvst(1:(K-3)/2,:);
% figure; plot(abs(S_fvst(bin,:)),'.--');
% zeta = max(abs(S_fvst(bin,:)))/max(abs(Y(54,:)));
% hold on; plot(zeta*abs(Y(54,2:end)),'o--')
...
for m=1:M
    for iter1=1:ITER1
        VarWsaveAll(m,iter1) = WsaveAll{iter1,m}(bin,:)*WsaveAll{iter1,m}(bin,:)';
    end
end
...
% For the final weighted sensor location image
...
% For the weighted sensor location movie
VarWsaveAdj = VarWsaveAll - repmat(min(VarWsaveAll),M,1);
VarWsaveAdj = VarWsaveAdj ./ repmat(max(VarWsaveAdj),M,1);
for iter1=ITER1%1:ITER1
    for m=1:M 
...
%% Look at covariance with diffuse noise single bin
figure; imagesc(abs(squeeze((R(bin,:,:)))))

%%
% fig=figure; 
% movie(fig,VarMov,1,25)

% figure; 
% image(VarMov(30).cdata)

%%
% node{89}.d(bin)'*node{89}.W(bin,node{89}.N==89)
...
%%
s5dB = s{1}*sqrt(10^(5/10));
s10dB = s{1}*sqrt(10^(10/10));
s15dB = s{1}*sqrt(10^(15/10));
s20dB = s{1}*sqrt(10^(20/10));
s25dB = s{1}*sqrt(10^(25/10));
s30dB = s{1}*sqrt(10^(30/10));
10*log10((s5dB.'*s5dB)/(s{2}.'*s{2}))
10*log10((s10dB.'*s10dB)/(s{2}.'*s{2}))
10*log10((s15dB.'*s15dB)/(s{2}.'*s{2}))
10*log10((s20dB.'*s20dB)/(s{2}.'*s{2}))
10*log10((s25dB.'*s25dB)/(s{2}.'*s{2}))
10*log10((s30dB.'*s30dB)/(s{2}.'*s{2}))
