...
%% Place sensors
M = 15^2; % M = number of sensors (note that the number of nodes will be 2*M to include virtual nodes)
...
% 2d grid placement
xloc = spSize*[combvec(linspace(0,1,sqrt(M)), linspace(0,1,sqrt(M)));repmat(1,1,M)];
...
for m=1:M
    node{m}.N = [find(sensd(:,m)<0.09*spSize) ; m+M]; % m+M is the virtual node for node m. Note that the scheme for listing neighbors is smallest number to largest, which means that the virtual node is always last in the neighbor list. It also means the self connection, i.e. where node m sits in node{m}.N can be anywhere except last.
...
for k=1:Khalf
    for ii=1:M
        for jj=1:M
            ...
            R(k,ii,jj) = exp(-1i*2*pi*fdomShort(k)*(temp1-temp2)/c)/((4*pi)^2 * temp1 * temp2) ;%+ 1e-2*sinc(fdomShort(k)*(temp1-temp2)/c);
        end
    end
    R(k,:,:) = squeeze(R(k,:,:)) ;%+ 1e-5*eye(M);
end
...
% ftmp = zeros(ITER1,M);
rho = 1.5; % scaling for consensus 1.5
beta = 1; % scaling for lambda consensus 1
