...
len_s = fs*30;%min(length(s1), length(s2));
...
% Ytmp = Y;
% Y = [zeros(1,L) ; conj(flipud(Ytmp)) ; zeros(1,L) ; Ytmp; zeros(1,L)];
% Y2 = [zeros(1,L) ; Ytmp ; zeros(1,L) ;conj(flipud(Ytmp)) ; zeros(1,L)];
% Y = [zeros(1,L);
%     Y;
%     zeros(1,L);
%     conj(flipud(Y));
