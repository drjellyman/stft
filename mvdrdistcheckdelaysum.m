...
[s1,fs1] = audioread('273177__xserra__la-vaca-cega-eva.wav'); % s1 = source, Fs1 = sampling frequency
[s2,fs2] = audioread('317354__speedenza__shall-i-compare-thee-voice.wav'); % s2 = noise stationary, Fs2 = sampling frequency
...
ls = 10; % length in seconds
ld = ls*fs-mod(ls*fs,K-1); % ld = desired length = seconds * seconds^-1
if lmin < ld
...
%% check delay and sum gain
Ydst = zeros(Khalf,L);
Ydsi = zeros(Khalf,L);
Yds = zeros(Khalf,L);
for l=1:L
    Ztmpt = squeeze(Zt(:,l,:));
    Ydst(:,l) = (1/M)*sum(conj(A) .* Ztmpt,2);
    Ztmpi = squeeze(Zi(:,l,:));
    Ydsi(:,l) = (1/M)*sum(conj(A) .* Ztmpi,2);
    Ztmp = squeeze(Z(:,l,:));
    Yds(:,l) = (1/M)*sum(conj(A) .* Ztmp,2);
   
end
W = conj(A);
% Create two sided Y for ifft
Ydst = [zeros(1,L);Ydst;zeros(2,L);conj(flipud(Ydst))];
Ydsi = [zeros(1,L);Ydsi;zeros(2,L);conj(flipud(Ydsi))];
Yds = [zeros(1,L);Yds;zeros(2,L);conj(flipud(Yds))];

% Take ifft
ydst = myOverlapAdd(Ydst);
ydsi = myOverlapAdd(Ydsi);
yds = myOverlapAdd(Yds);
figure; plot(yds);

ydstPow = sqrt(ydst'*ydst)
ydsiPow = sqrt(ydsi'*ydsi)
ydsPow = sqrt(yds'*yds)
tiPow = ydstPow/ydsiPow
tiPowdB = 10*log10(tiPow)
